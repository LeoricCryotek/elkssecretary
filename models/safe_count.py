# -*- coding: utf-8 -*-
"""Safe / bank count model — denomination-level cash count for the safe.

Same denomination breakdown as till counts but represents the house bank
(safe in the back).  One safe count per Treasury Session.
"""
from odoo import api, fields, models, _
from odoo.exceptions import UserError

from .denomination_line import DENOMINATION_SELECTION


class ElksSafeCount(models.Model):
    """Cash count for the house bank / safe."""

    _name = "elks.safe.count"
    _description = "Safe / Bank Count"
    _order = "session_id desc, id desc"
    ...
    denomination_ids = fields.One2many(
        "elks.denomination.line", "safe_count_id",
    ...
    # ── computes ─────────────────────────────────────────────────
    @api.depends("session_id.session_date")
    def _compute_name(self):
        for rec in self:
            date = rec.session_id.session_date or ''
            rec.name = f"Safe Count — {date}" if date else "Safe Count"
    ...
    def action_done(self):
        ...
        for rec in self:
            ...
            rec.message_post(
                body=_(
                    "<strong>Safe Count Complete</strong><br/>"
                    "Total: $%(total).2f",
                    total=rec.total,
                ),
